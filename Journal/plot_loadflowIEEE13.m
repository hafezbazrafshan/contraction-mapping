clear all;
cd('Results'); 
zbusResults=load('IEEE13ZBusResults'); 
sweepResults=load('IEEE13SWeepResults');
cd('..'); 
...
for phase=1:3
    ...
    ylim([0.9 1.2]);
    grid on;
		set(gca,'XTick', [1:1:length(allNodesActualLabelsWithRegs)-2, length(allNodesActualLabelsWithRegs)-1],'YTick', [0.95:0.05:1.05] );
    set(gca,'XTickLabel', [allNodesActualLabelsWithRegs(1:1:end-2) ;allNodesActualLabelsWithRegs(end-1)]);
...
cd('Figures'); 
print -dpdf IEEE13magnitudes.pdf
print -depsc2 IEEE13magnitudes
cd('..');
...
for phase=1:3
    ...
    grid on;
		set(gca,'XTick', [1:1:length(allNodesActualLabelsWithRegs)-2, length(allNodesActualLabelsWithRegs)-1]);
    set(gca,'XTickLabel', [allNodesActualLabelsWithRegs(1:1:end-2) ;allNodesActualLabelsWithRegs(end-1)]);
    legendTEXT=legend('FB-Sweep', 'Z-BUS ');
    set(legendTEXT,'interpreter','Latex'); 
set(legendTEXT,'fontSize',14); 
...
cd('Figures');
print -dpdf IEEE13phases.pdf
print -depsc2 IEEE13phases
cd('..'); 