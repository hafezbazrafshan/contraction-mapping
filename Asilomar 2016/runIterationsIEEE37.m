...
% lambdaMat=eye(length(IEEE123.w));
% Initialization 1
...
[ alphaMax3, alphaNumeric3,success3, vNew3, vIterations3, err3 ] = ZBUSV(IEEE123,vPr3);
  ...
  %Initialization 3
vPr4=IEEE123.w-validRangeIEEE123(end-22)*IEEE123.w;
[ alphaMax4, alphaNumeric4, success4, vNew4, vIterations4, err4 ] = ZBUSV(IEEE123,vPr4);
...
ylabel('$\|  \mathbf{v}[t] - \mathbf{w} \|_{\infty}$'); 
...
hold on
 ...
 h4=plot(1:maxIt-1, alphaNumeric4, 'm-v','lineWidth',2); 
 hold on;
h5=plot(1:maxIt-1,repmat(min(alphaTheoryVecIEEE123), length(alphaNumeric1),1),'k-*','lineWidth',2);
 hold on
...
set(currentAxes,'XTick',1:maxIt-1);
set(currentAxes,'XtickLabels',1:maxIt-1);
set(currentAxes,'YTick',[0.04:0.02: 0.26]);
xlim([0 maxIt]);
 ylim([0.04 0.3]);
...
set(currentAxes,'FontName','Times New Roman');
ylabel('$\frac{\| \mathbf{\Lambda}^{-1}(\mathbf{v}[t+1] - \mathbf{v}[t])\|_{\infty}}{\| \mathbf{\Lambda}^{-1}(\mathbf{v}[t] - \mathbf{v}[t-1])\|_{\infty}} $'); 
legendTEXT=legend([h1, h2, h3, h4,h5], '$\mathbf{v}_n[0]=\mathbf{v}_{\mathrm{S}}$', '$\mathbf{v}_n[0]=\mathbf{w}_n$', ...
    '$\mathbf{v}_n[0]=\mathbf{w}_n-R_{\min}\mathbf{w}_n$', '$\mathbf{v}_n[0]=\mathbf{w}_n-R_{\max} \mathbf{w}_n$', '$\alpha_{\min}$');
set(legendTEXT,'interpreter','Latex'); 
set(legendTEXT,'fontSize',14); 
set(legendTEXT,'fontname','Times New Roman');
set(legendTEXT,'fontWeight','Bold');
